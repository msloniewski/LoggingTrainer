...
class SpeechEngine:
    def __init__(self, gap_ms: int = 35, asset_dir: Path = ASSET_DIR) -> None:
        self._items: queue.Queue[tuple[str, int, dict[str, str]] | None] = queue.Queue()
        self._gap_ms = gap_ms
    ...
    def say(self, callsign: str, speed: int = 1) -> None:
        ...
        self._items.put((callsign, speed, self._pronunciations))
    ...
    def _validate(self) -> str | None:
        missing = [
            ...
            if not self._clip(character, word, variant).exists()
        ]
        if missing:
    ...
    def _clip(self, character: str, word: str, speed: int) -> Path:
        if character == "/":
            return self._asset_dir / f"stroke-{speed}.wav"
        slug = word.lower().replace(" ", "-")
        return self._asset_dir / f"{character.upper()}-{slug}-{speed}.wav"
    ...
    def _build_wav(
        ...
        clips = [
            self._clip(character, pronunciations[character], speed)
    ...
    def _worker(self) -> None:
        while True:
            ...
            if item is None:
                return
            callsign, speed, pronunciations = item

            path: Path | None = None
            try:
                with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as temporary:
                    path = Path(temporary.name)
                self._build_wav(callsign, path, speed, pronunciations)
                self._play(path)
